...
if [[ "${DEPS}" == "1" ]]; then
  ...
  if [ "${OS}" == "Linux" ]; then
    ...
    brew install qt5 imagemagick && brew link --force qt5 || exiterr "deps failed (mac), exiting."
  else
...
if [[ "${BUILD}" == "1" ]]; then
  ...
  qmake && make ${MAKEARGS} || exiterr "build failed, exiting."
fi
...
if [[ "${DOC}" == "1" ]]; then
  if [[ -x "$(command -v help2man)" ]]; then
    if [[ "$(uname)" == "Darwin" ]]; then
      SED="gsed -i"
      PROGPATH="./namp.app/Contents/MacOS/nview"
    else
      SED="sed -i"
      PROGPATH="./nview"
    fi

    help2man -n "nano view" -N -o ./res/nview.1 ${PROGPATH} && ${SED} "s/\.\\\\\" DO NOT MODIFY THIS FILE\!  It was generated by help2man.*/\.\\\\\" DO NOT MODIFY THIS FILE\!  It was generated by help2man./g" ./res/nview.1 || exiterr "doc failed, exiting."
  fi
fi
